...
fn main(@builtin(workgroup_id) wgid: vec3<u32>, @builtin(local_invocation_id) lid: vec3<u32>) {
    ...
    loop {
        ...
        // Load A tile into shared_a[32*16]: 2 elements per thread
        ...
        // Load B tile into shared_b[16*32]: 2 elements per thread
        // shared_b layout: [k_local * 32 + n_local]
        for (var e = 0u; e < 2u; e++) {
            let flat = tid + e * 256u;
            ...
            shared_b[row_local * 32u + col_local] = select(0.0, matrix_b[$B_INDEX], in_bounds);
        }
